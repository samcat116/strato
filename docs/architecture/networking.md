...
- **Metadata reachability** — the localport, its per-chassis namespace
  ...
  egress allow (STR-54) all exist, but nothing listens on the *metadata*
  addresses (STR-56). The namespace itself is no longer empty: STR-40 runs a
  CoreDNS in it for the resolver addresses, which is the first user of the
  per-namespace helper-process shape ADR 0003 left to STR-56. Until STR-56 lands, the advertised route points at an address with
