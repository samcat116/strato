...
| `GUEST_IDENTITY_AUDIENCES` | Comma-separated relying-party audiences (at most 255 characters each) for which a hosting agent may mint a VM JWT-SVID through `POST /agent/vms/{vmID}/jwt-svid`. Empty disables guest issuance. | empty (issuance off) |
