...
extension Agent: ReconcileActuator {
    ...
    func adoptVM(_ item: ReconcileWorkItem) async throws -> VMStatus {
        ...
        let status: VMStatus
        do {
            status = try await StageBudget.run(
                seconds: StageBudget.adoptionSeconds,
                stage: "adopt VM \(item.id)",
                onTimeout: .abandon
            ) {
                try await service.adoptVM(vmId: item.id, spec: entry.spec)
            }
    ...
    func adoptSandbox(_ item: ReconcileWorkItem) async throws -> SandboxStatus {
        ...
        do {
            status = try await StageBudget.run(
                seconds: StageBudget.adoptionSeconds,
                stage: "adopt sandbox \(item.id)",
                onTimeout: .abandon
            ) {
                try await runtime.adoptSandbox(
