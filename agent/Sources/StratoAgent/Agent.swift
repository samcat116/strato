...
actor Agent {
    ...
    private func getRunningVMList() async -> [String] {
        ...
        for (type, service) in hypervisorServices {
            ...
            // whether the poll timed out or the backend could not say (STR-196).
            // Nothing under `control-plane/` reads
            // `AgentHeartbeatMessage.runningVMs` today — said plainly so this
            // comment is not mistaken for a description of live behavior again
            // — and the fallback stays anyway, because a *short* list is the
            // wrong kind of wrong for an inventory of what exists on a host.
            // Absence reads as gone, and the field's next consumer will read it
            // that way.
            let vms =
