...
public enum FirmwareResolver {
    ...
    public struct UnresolvedError: Error, CustomStringConvertible, Equatable, Sendable {
        ...
        public init(secureBoot: Bool, architecture: CPUArchitecture) {
    ...
    public static func defaultMonolithicPath(architecture: CPUArchitecture) -> String? {
        defaultMonolithicCandidates(architecture: architecture).first {
            FileManager.default.fileExists(atPath: $0)
        }
    }

    private static func defaultMonolithicCandidates(architecture: CPUArchitecture) -> [String] {
        #if os(macOS)
        switch architecture {
        case .arm64:
