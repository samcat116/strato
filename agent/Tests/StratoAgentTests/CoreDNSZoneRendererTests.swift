...
struct CoreDNSZoneRendererTests {
    ...
    func forwardZoneShape() throws {
        ...
        let body = try #require(file(rendering, "zones/corp.example.com.zone"))
    ...
    func reverseZoneIsSynthesized() throws {
        ...
        let forward = try #require(file(rendering, "zones/corp.example.com.zone"))
        #expect(!forward.contains("in-addr.arpa"))

        let reverse = try #require(file(rendering, "zones/0.0.10.in-addr.arpa.zone"))
    ...
    func reverseGroupsPerTwentyFour() {
        ...
        #expect(paths.contains("zones/0.0.10.in-addr.arpa.zone"))
        #expect(paths.contains("zones/1.0.10.in-addr.arpa.zone"))
    }
    ...
    func reverseGroupsPerSixtyFour() throws {
        ...
        #expect(rendering.files.contains { $0.relativePath == "zones/\(origin).zone" })
    }
    ...
    func corefileBindsResolverAddressesOnly() throws {
        // The namespace holds the metadata addresses too; a wildcard listener
        // would answer DNS on 169.254.169.254:53, an address whose contract is
        // HTTP metadata.
        let rendering = CoreDNSZoneRenderer.render(
            zones: [zone(records: [])], upstreams: ["1.1.1.1"], searchDomain: nil)
        let corefile = try #require(file(rendering, "Corefile"))
        #expect(corefile.contains("bind 169.254.169.253 fd00:ec2::253"))
    ...
    func bindListIsCallerSupplied() throws {
        ...
        // exist, so hardcoding the ULA would take the whole resolver down on a
        // host where the namespace's v6 `addr add` failed and v4 works fine.
        let rendering = CoreDNSZoneRenderer.render(
            zones: [], upstreams: [], searchDomain: nil,
            bindAddresses: [NetworkResolverEndpoint.address])
        let corefile = try #require(file(rendering, "Corefile"))
        #expect(corefile.contains("bind 169.254.169.253"))
        #expect(!corefile.contains("fd00:ec2::253"))
    }
    ...
    func forwardingIsOptional() throws {
        let withUpstreams = try #require(
            file(
                CoreDNSZoneRenderer.render(zones: [], upstreams: ["1.1.1.1", "8.8.8.8"], searchDomain: nil),
                "Corefile"))
        ...
        let without = try #require(
            file(CoreDNSZoneRenderer.render(zones: [], upstreams: [], searchDomain: nil), "Corefile"))
    ...
    func everyZoneIsServed() throws {
        ...
        #expect(corefile.contains("file zones/corp.example.com.zone"))
        #expect(corefile.contains("0.0.10.in-addr.arpa:53 {"))
        #expect(corefile.contains("file zones/0.0.10.in-addr.arpa.zone"))
    ...
    func zonesAutoReload() throws {
        ...
        let rendering = CoreDNSZoneRenderer.render(
            zones: [zone(records: [])], upstreams: [], searchDomain: nil)
        let corefile = try #require(file(rendering, "Corefile"))
    ...
    func searchDomainIsRecorded() throws {
        let good = try #require(
            file(
                CoreDNSZoneRenderer.render(zones: [], upstreams: [], searchDomain: "corp.example.com"),
                "Corefile"))
        #expect(good.contains("# network search domain: corp.example.com"))

        let bad = try #require(
            file(CoreDNSZoneRenderer.render(zones: [], upstreams: [], searchDomain: "not a domain"), "Corefile"))
    ...
    func serialFollowsRenderedContents() throws {
        func body(hash: String, records: [DesiredDNSRecord]) throws -> String {
            try #require(
                file(
                    CoreDNSZoneRenderer.render(
                        zones: [zone(records: records, hash: hash)], upstreams: [], searchDomain: nil),
                    "zones/corp.example.com.zone"))
        }
    ...
    func reverseSerialTracksTTL() throws {
        func reverse(ttl: Int) throws -> String {
            let rendering = CoreDNSZoneRenderer.render(
                ...
                upstreams: [], searchDomain: nil)
            return try #require(file(rendering, "zones/0.0.10.in-addr.arpa.zone"))
        }
    ...
    func renderIsDeterministic() {
        ...
        let first = CoreDNSZoneRenderer.render(
            zones: [zone(records: records)], upstreams: ["1.1.1.1"], searchDomain: "corp.example.com")
        let second = CoreDNSZoneRenderer.render(
    ...
    func invalidZoneNameIsSkipped() {
        let rendering = CoreDNSZoneRenderer.render(
            zones: [zone(name: "not a zone", records: [])], upstreams: [], searchDomain: nil)
