...
struct VsockConnectionTests {
    ...
    @Test("close() completes a read waiting on the connection")
    func closeUnblocksParkedRead() async throws {
        ...
        // inbound data has nothing to complete it until the close.
        ...
        let readTask = Task { try await conn.nextLine() }
        // No settling sleep is needed any more: the channel owns the
        // descriptor, so close() is well-defined whether the read has been
        // registered yet or not. Previously this had to wait 200ms to make the
        // race "probably" safe.
        await conn.close()

        // The waiting read must complete with nil (peer closed) rather than
        // hanging, so race a generous deadline and fail instead of blocking the
        // suite if that regresses.
        enum Outcome: Sendable { case read(String??), timedOut }
        ...
        guard case .read(let line) = outcome else {
            Issue.record("nextLine() never completed after close()")
            return
        }
        // nil is the expected completion: no line arrived before the close.
        #expect(line ?? nil == nil)
    }
