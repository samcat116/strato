...
/// copies had drifted over whether the resolved project must exist — four
/// confirmed it, `createVolume` left the row insert to trip the foreign key.
/// These pin the consolidated behaviour: the same `400` from every endpoint,
/// the same fallback, and the same two refusals when there is nothing to fall
/// back to.
...
final class ProjectResolutionTests {
    ...
    func omittedProjectFallsBackToTheOrganizationsProject() async throws {
        try await withProjectResolutionApp { app, _, _, _, project, token in
            try await app.test(.POST, "/api/networks") { req in
                req.headers.bearerAuthorization = BearerAuthorization(token: token)
