...
final class QuotaEnforcementTests {
    ...
    func applicableQuotasResolvesHierarchy() async throws {
        try await withApp { app, _, org, _, _, _ in
            ...
            let resolved = try await QuotaEnforcementService.applicableQuotas(
                for: project, environment: "development", on: app.testPostgres)
    ...
    func applicableQuotasHonorsEnvironment() async throws {
        try await withApp { app, _, _, project, _, _ in
            ...
            let resolved = try await QuotaEnforcementService.applicableQuotas(
                for: project, environment: "production", on: app.testPostgres)
            let ids = Set(resolved.compactMap { $0.id })
    ...
    func reserveEnforcesAncestorOUQuota() async throws {
        try await withApp { app, _, org, _, _, _ in
            ...
            try await QuotaEnforcementService.reserve(
                ...
                vcpus: 3, memory: gb(2), storage: gb(10), on: app.testPostgres)
            let afterReserve = try await ResourceQuota.find(engQuota.id, on: app.testPostgres)!
            ...
            await #expect(throws: Abort.self) {
                try await QuotaEnforcementService.reserve(
                    for: project, environment: "development",
                    vcpus: 3, memory: gb(1), storage: gb(1), on: app.testPostgres)
            }
        }
    }
    ...
    func intermediateOUQuotaReportsUsage() async throws {
        try await withApp { app, _, org, _, _, _ in
            ...
            let usage = try await engQuota.calculateActualUsage(on: app.testPostgres)
    ...
    func reserveIncrementsQuotas() async throws {
        try await withApp { app, _, org, project, _, _ in
            ...
            try await QuotaEnforcementService.reserve(
                for: project, environment: "development",
                vcpus: 3, memory: gb(4), storage: gb(20), on: app.testPostgres)

            let refreshedOrg = try await ResourceQuota.find(orgQuota.id, on: app.testPostgres)!
            let refreshedProj = try await ResourceQuota.find(projQuota.id, on: app.testPostgres)!
    ...
    func reserveRejectsWhenExceeded() async throws {
        try await withApp { app, _, _, project, _, _ in
            ...
            await #expect(throws: Abort.self) {
                try await QuotaEnforcementService.reserve(
                    for: project, environment: "development",
                    vcpus: 4, memory: gb(1), storage: gb(1), on: app.testPostgres)
            }
        }
    }
    ...
    func reserveIsAllOrNothing() async throws {
        try await withApp { app, _, org, project, _, _ in
            ...
            let refreshed = try await ResourceQuota.find(roomyOrg.id, on: app.testPostgres)!
    ...
    func disabledQuotaTracksButDoesNotBlock() async throws {
        try await withApp { app, _, _, project, _, _ in
            ...
            let refreshed = try await ResourceQuota.find(quota.id, on: app.testPostgres)!
    ...
    func releaseDoesNotEraseOtherReservations() async throws {
        try await withApp { app, _, _, project, _, _ in
            ...
            try await QuotaEnforcementService.reserve(
                for: project, environment: "development",
                vcpus: 2, memory: gb(2), storage: gb(10), on: app.testPostgres)
            _ = try await builder.createVM(name: "b", project: project)  // cpu 2

            let afterReserve = try await ResourceQuota.find(quota.id, on: app.testPostgres)!
            ...
            try await vmA.delete(on: app.testPostgres)
            try await QuotaEnforcementService.release(for: vmA, on: app.testPostgres)

            let afterDelete = try await ResourceQuota.find(quota.id, on: app.testPostgres)!
    ...
    func reserveSandboxSharesPools() async throws {
        try await withApp { app, _, _, project, _, _ in
            ...
            try await QuotaEnforcementService.reserveSandbox(
                for: project, environment: "development",
                vcpus: 3, memory: gb(4), on: app.testPostgres)
    ...
    func vmAndSandboxShareVCPUPool() async throws {
        try await withApp { app, _, _, project, _, _ in
            ...
            await #expect(throws: Abort.self) {
                try await QuotaEnforcementService.reserveSandbox(
                    for: project, environment: "development",
                    vcpus: 3, memory: gb(1), on: app.testPostgres)
            }

            // A sandbox that fits the remaining 2 vCPUs is admitted.
            try await QuotaEnforcementService.reserveSandbox(
                for: project, environment: "development",
                vcpus: 2, memory: gb(1), on: app.testPostgres)
        }
    }
    ...
    func sandboxCountLimitEnforced() async throws {
        try await withApp { app, _, _, project, _, _ in
            ...
            try await QuotaEnforcementService.reserveSandbox(
                for: project, environment: "development",
                vcpus: 1, memory: gb(1), on: app.testPostgres)
            ...
            try await QuotaEnforcementService.reserve(
                for: project, environment: "development",
                vcpus: 1, memory: gb(1), storage: gb(1), on: app.testPostgres)
        }
    }
    ...
    func sandboxReleaseRecomputes() async throws {
        try await withApp { app, _, _, project, _, _ in
            ...
            try await QuotaEnforcementService.release(for: vm, on: app.testPostgres)
            let before = try await ResourceQuota.find(quota.id, on: app.testPostgres)!
            ...
            try await QuotaEnforcementService.release(for: sandbox, on: app.testPostgres)

            let after = try await ResourceQuota.find(quota.id, on: app.testPostgres)!
    ...
    func sandboxCreateRejectedWhenQuotaExceeded() async throws {
        try await withApp { app, _, _, project, _, token in
            ...
            let sandboxCount = try await Sandbox.all(on: app.testPostgres).count
    ...
    func createRejectedWhenQuotaExceeded() async throws {
        try await withApp { app, _, _, project, image, token in
            ...
            #expect(refreshed.reservedVCPUs == 0)
            #expect(refreshed.vmCount == 0)
            let vmCount = try await VM.all(on: app.testPostgres).count
            #expect(vmCount == 0)
        }
    }
    ...
    func createReservesAndDeleteReleases() async throws {
        try await withApp { app, _, _, project, image, token in
            ...
            let afterCreate = try await ResourceQuota.find(quota.id, on: app.testPostgres)!
            ...
            // write the same row.
            try await waitForCreateToSettle(vmID: createdVMID!, on: app.testPostgres)
            ...
            try await waitForVMDeletion(createdVMID!, on: app.testPostgres)
            ...
            #expect(
                try await Volume.all(on: app.testPostgres).count { $0.vmID == createdVMID! } == 0)
        }
    }
    ...
    private func waitForCreateToSettle(vmID: UUID, on db: PostgresStoreContext) async throws {
        for _ in 0..<100 {
    ...
    private func waitForVMDeletion(_ vmID: UUID, on db: PostgresStoreContext) async throws {
        for _ in 0..<100 {
