...
/// Unit tests for `JWK.createRSAPublicKey` — the hand-rolled ASN.1/DER
/// encoding that turns a JWKS modulus/exponent pair into a PEM public key.
@Suite("JWK RSA Public Key Construction Tests")
struct JWKRSAKeyTests {

    private func makeJWK(n: String = TestRSAKeys.modulus, e: String = TestRSAKeys.exponent) -> App.JWK {
        App.JWK(kty: "RSA", use: "sig", kid: TestRSAKeys.keyID, n: n, e: e, alg: "RS256")
    }

    @Test("A key built from JWK n/e verifies a token signed by the matching private key")
    func testRoundTripVerification() throws {
        let token = try signIDToken(nonce: "n-1")

        let publicKey = try makeJWK().createRSAPublicKey()
        let signers = JWTSigners()
        signers.use(.rs256(key: publicKey), kid: JWKIdentifier(string: TestRSAKeys.keyID))

        let claims = try signers.verify(token, as: TestIDTokenClaims.self)
        #expect(claims.sub == "sub-123")
        #expect(claims.nonce == "n-1")
    }

    @Test("A key built from JWK n/e rejects a token signed by a different private key")
    func testRejectsForeignSignature() throws {
        let token = try signIDToken(nonce: "n-1", privateKeyPEM: TestRSAKeys.otherPrivateKeyPEM)
        ...
        #expect(throws: (any Error).self) {
            _ = try signers.verify(token, as: TestIDTokenClaims.self)
        }
    }

    @Test("A key built from JWK n/e rejects a tampered payload")
    func testRejectsTamperedPayload() throws {
        let token = try signIDToken(nonce: "n-1")

        // Swap the payload segment for one claiming a different subject while
        ...
        let forgedPayload = try JSONSerialization.data(
            withJSONObject: ["sub": "someone-else", "exp": Int(Date().timeIntervalSince1970) + 300])
        parts[1] = forgedPayload.base64EncodedString()
        ...
        #expect(throws: (any Error).self) {
            _ = try signers.verify(tampered, as: TestIDTokenClaims.self)
        }
    }

    @Test("The DER encoding matches BoringSSL's own JWK import")
    func testDERMatchesReferenceImplementation() throws {
        // JWTKit can build an RSA key straight from the JWK parameters via
        // BoringSSL. Both constructions must verify the same signature,
        // proving the hand-rolled DER path encodes n/e faithfully.
        let token = try signIDToken(nonce: "n-1")
        let reference = try #require(RSAKey(modulus: TestRSAKeys.modulus, exponent: TestRSAKeys.exponent))
        let handRolled = try makeJWK().createRSAPublicKey()

        for key in [reference, handRolled] {
            let signers = JWTSigners()
            signers.use(.rs256(key: key), kid: JWKIdentifier(string: TestRSAKeys.keyID))
            let claims = try signers.verify(token, as: TestIDTokenClaims.self)
            #expect(claims.sub == "sub-123")
        }
    }

    @Test("Invalid base64url modulus or exponent throws instead of crashing")
    func testInvalidBase64Throws() {
        #expect(throws: (any Error).self) {
            _ = try makeJWK(n: "!!!not-base64url!!!").createRSAPublicKey()
        }
        #expect(throws: (any Error).self) {
            _ = try makeJWK(e: "???").createRSAPublicKey()
        }
    }
}
