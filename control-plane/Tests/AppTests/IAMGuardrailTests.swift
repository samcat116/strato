...
final class IAMGuardrailTests {
    ...
    @Test("A guardrail forbidding everything to everyone is refused")
    func totalGuardrailRefused() async throws {
        try await withApp { app in
            let tree = try await buildTree(TestDataBuilder(db: app.db), prefix: "Total")

            await #expect(throws: GuardrailError.forbidsEverything) {
                _ = try await GuardrailStore.create(
