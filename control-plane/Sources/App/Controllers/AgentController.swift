...
struct AgentController: RouteCollection {
    ...
    func reassignOrganization(req: Request) async throws -> AgentResponse {
        ...
        // Detached volumes anchor the old org's data to this hardware through
        // their replica rows, the same way VMs do through placement.
        let storedVolumes = try await VolumeReplica.query(on: req.db)
            .filter(\.$agentId == agentId.uuidString)
