...
///
/// `EnforcePersistedEnumValues` installed a CHECK constraint for the column;
/// deployments that migrated before this kind existed would reject every
/// export operation insert at the database. Re-installing the constraint with
/// the extended list is the documented follow-up for adding an enum case —
/// install is idempotent (drop-if-exists first), so fresh databases whose base
/// migration already carried the new value are unaffected.
