...
enum IPAMService {
    ...
    /// `(logical_network_id, address)` unique indexes only catch same-table
    /// races: a concurrent VM create and sandbox create insert into different
    /// tables, so neither hits a constraint and both could commit the same
    /// address. With the lock, the second allocator waits until the first
    /// transaction commits and then reads a used set that includes the winner's
    /// row.
    ///
    /// Keyed on the network's id, not its name: names are unique only within a
    ...
    static func allocateIP(for network: LogicalNetwork, on db: Database) async throws -> Allocation {
        ...
        // Each table's own `(logical_network_id, address)` unique index
        // backstops concurrent same-table creates; cross-table (VM vs sandbox)
