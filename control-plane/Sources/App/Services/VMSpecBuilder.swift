...
struct VMSpecBuilder {
    /// Upper bound on a guest kernel cmdline. Matches the VM-create API guard.
    ...
    private static func ensureSerialConsole(_ cmdline: String?) -> String {
        ...
        let cleaned = (cmdline ?? "").unicodeScalars
            .filter { $0.value == 0x20 || ($0.value > 0x20 && $0.value != 0x7f) }
            .prefix(maxCmdlineLength)
        let trimmed = String(cleaned.map(Character.init)).trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.isEmpty {
