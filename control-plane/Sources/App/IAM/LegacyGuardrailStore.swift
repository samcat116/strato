...
enum LegacyGuardrailStore {
    static func guardrail(id: UUID, on db: any Database) async throws -> IAMGuardrailSnapshot? {
    ...
    static func missingCedarText(on db: any Database) async throws -> [IAMGuardrailSnapshot] {
        try await requireSQL(db).raw(
