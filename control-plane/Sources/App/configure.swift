...
public func configure(_ app: Application) async throws {
    ...
    // At-rest encryption for recoverable secrets (OIDC client secrets). A
    // malformed key fails startup — a typo must not silently downgrade to
    // plaintext storage — while an absent key runs pass-through with a warning
    // so existing deployments keep working until the operator sets one.
    ...
    if !secretsEncryption.isEnabled {
        app.logger.warning(
            "STRATO_SECRET_ENCRYPTION_KEY is not set — OIDC client secrets will be stored unencrypted. Generate a key with `openssl rand -hex 32` and set it to enable encryption at rest."
        )
    }
    ...
    // Converge any plaintext OIDC client secrets to encrypted form. Runs every
    // startup (not a one-shot migration) so a key added after upgrade still
    // picks up rows written before it existed. No-op without a key.
    try await secretsEncryption.encryptStoredOIDCClientSecrets(on: app.db, logger: app.logger)
