  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="max-h-96 divide-y overflow-auto rounded-lg border">{(query.data ?? []).map((registration) => <div key={registration.id} className="flex items-center justify-between gap-4 p-3"><div className="min-w-0"><p className="truncate font-mono text-sm">{registration.spiffeId}</p><p className="text-xs text-muted-foreground">{registration.kind} · {registration.displayName ?? registration.agentName ?? "Unlabelled"}</p></div><Button variant="ghost" size="icon" aria-label={`Delete registration ${registration.spiffeId}`} onClick={() => remove.mutate(registration.id)}><Trash2 className="h-4 w-4" /></Button></div>)}</div>
      </CardContent>
    </Card>
  );
}
