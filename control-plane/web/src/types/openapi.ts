...
export interface paths {
    ...
    "/api/vms/{vmID}": {
         ...
         * @description Updates `name`/`description`, and `cpu`/`memory` (issue #568). vCPU growth and memory changes on a *running* VM are applied online within the `maxCpu`/`maxMemory` ceilings it was started with and answer `202` with a `resize` operation. A vCPU shrink from confirmed live sizing is rejected because live vCPU unplug is not supported. A smaller target may supersede a pending growth request; the agent refuses convergence if the live count has already grown past it. A sizing change to a stopped VM (which may also raise those ceilings, since the next boot re-spawns the VM) and a metadata-only update answer `200`.
         */
...
export interface components {
    schemas: {
        ...
        UpdateVMRequest: {
            ...
            hostname?: string;
            description?: string;
            /** @description Target boot vCPU count. On a running VM with confirmed sizing it must be at least the current count and must not exceed `maxCpu`; reduce it while stopped. A smaller target may replace growth that has not converged yet. */
            cpu?: number;
            /**
             * Format: int64
        ...
        Volume: {
            ...
            blockMode: components["schemas"]["VolumeBlockMode"];
            /** @description Exact attributes selected by the agent. Omitted until an agent supporting wire v59 reports; `active: false` explicitly means the volume is detached. `fallbackReason` explains any safe downgrade. */
...
export interface operations {
    ...
    updateVM: {
        ...
        responses: {
            ...
            404: components["responses"]["NotFound"];
            409: components["responses"]["Conflict"];
            /** @description The requested size exceeds the ceilings the running VM was started with, its agent is too old to resize online, or the request would reduce a running VM's confirmed live vCPU count. Stop the VM before reducing vCPUs; otherwise restart it to raise its online-resize ceiling. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
