...
public enum WireProtocol {
    ...
    ///
    /// Version 13: sandbox snapshot mobility (issue #428). Adds the
    /// `sandboxSnapshotExport` request/response pair (a new `MessageType`
    /// case, so like v6/v9 the send-side gate is load-bearing: a pre-v13
    /// agent drops the undecodable envelope and the request would burn its
    /// timeout against silence), optional `artifacts` download descriptors on
    /// `SandboxRestoreMessage` and `SandboxSnapshotRef` (a pre-v13 agent
    /// ignores them and would fail the restore with "snapshot not found", so
    /// cross-agent restore/fork placement is refused for such agents), and
    /// `SandboxSpec.cpuTemplate` (silently ignored by a pre-v13 agent — the
    /// sandbox would boot un-templated while the API reports a template, so
    /// templated creates are gated too; see
    ...
    /// export to object storage, artifact download descriptors on restore and
    /// fork, and CPU templates on sandbox specs (see `currentVersion` version
    /// 13 notes).
    public static let sandboxSnapshotMobilityMinimumVersion = 13

    /// Whether an agent registered with `version` can be sent a
    /// `SandboxSnapshotExportMessage`, an `artifacts`-carrying restore/fork,
    /// or a templated `SandboxSpec`. A pre-v13 agent either cannot decode the
