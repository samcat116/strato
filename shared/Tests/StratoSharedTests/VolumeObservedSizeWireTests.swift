...
/// The wire contract behind `ObservedVolumeState.sizeBytes` (STR-199, v37).
///
...
/// nothing*. A pre-v37 agent omits the key on every report, and the control
...
struct VolumeObservedSizeWireTests {
    ...
    /// A v36 payload — no `sizeBytes` key at all — must still decode, and must
    /// decode as "no answer" rather than failing the whole report.
    @Test func aPreV37ObservedVolumeStateDecodesWithNoSize() throws {
        let json = """
            {
    ...
    /// A volume the agent could not probe is reported the same way a pre-v37
    ...
    @Test func areportedSizeSurvivesTheRoundTrip() throws {
        let observed = ObservedVolumeState(
            volumeId: UUID(), present: true, storagePath: "/v/volume.qcow2", format: "qcow2",
