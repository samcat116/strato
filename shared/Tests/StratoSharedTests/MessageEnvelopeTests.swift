...
struct MessageEnvelopeTests {
    ...
    func envelopeTypeMatchesMessage() throws {
        let message = VMOperationMessage(type: .vmBoot, vmId: "vm-1")
        let envelope = try MessageEnvelope(message: message)
        #expect(envelope.type == .vmBoot)
    }
    ...
    func fullRoundTrip() throws {
        ...
        #expect(decoded.details == message.details)
    }
    ...
    func wireShape() throws {
        ...
        #expect(json["type"] as? String == "vm_info")
        ...
        let inner = try decodeJSON(VMInfoRequestMessage.self, from: payload)
