...
# --- Secrets (generated; keep database/Valkey passwords stable after first
# --- start, and rotate the encryption key only with the procedure below) ---
POSTGRES_PASSWORD=$(openssl rand -hex 32)
VALKEY_PASSWORD=$(openssl rand -hex 32)
# Encrypts recoverable stored secrets at rest in the database. If this key is
# lost, those secrets become unreadable. Rotate it with the previous-key
# procedure in docs/deployment/docker-compose.md; never replace it by itself.
STRATO_SECRET_ENCRYPTION_KEY=$(openssl rand -hex 32)
# Comma-separated decrypt-only keys. Empty until a rotation is in progress.
STRATO_SECRET_ENCRYPTION_KEYS_PREVIOUS=
