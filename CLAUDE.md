...
- Imperative actions that aren't states (VM reboot, VM/sandbox restore) forward over `replica:{id}:rpc` channels. A volume's own lifecycle left that list in STR-148 and every snapshot artifact's in STR-150. Scheduler placement reservations (`resv:*`) and singleton sweep locks (`lock:sweep:*`) also live in Valkey.
