...
- The `replica:{id}:rpc` forwarding channels have **no sender left**: volumes left in STR-148, snapshot artifacts in STR-150, and VM reboot / VM+sandbox restore in STR-151 (counted as nonces instead). Console and exec write straight to the local socket, so the machinery is reachable only from itself and retires in STR-152. Scheduler placement reservations (`resv:*`) and singleton sweep locks (`lock:sweep:*`) still live in Valkey.
...
- **Reboot and restore are edge-nonces** (ADR 0001 stage 9, STR-151, wire v34): `DesiredVMState.rebootGeneration`, `DesiredVMState.restore` and `DesiredSandboxState.restore` count how many times each verb was asked for, and the agent acts only when the count outranks the one it recorded in `VMManifestStore`. **No record is not zero** — an entry from an older build is adopted, never replayed, or a re-registered agent would rewind a live guest to a checkpoint from weeks ago. An edge is consumed by being *superseded* (a stop, or a boot) as much as by being performed. `supportsEdgeNonces` refuses the three endpoints with `409` against a pre-v34 agent, which would otherwise ignore the field and report the generation as converged.
